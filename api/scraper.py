...
def FetchEvents():
    ...
    try:
        containers = soup.find_all("div", {"class":"c-venue-events-list"})
        for container in containers:
            shows = container.find_all("div", {"class":"c-venue-events-list__shows"})
            for s in shows:
                li = s.find_all('li')
                for l in li:
                    title = l.find('h1', {"class":"h1 h1--title-card"})
                    location = l.find('p', {"class":"text-overflow"})
                    date = l.find('div', {"class":"o-date"})
                    link = l.find("a", href=True)['href']   
                    img = l.find("img")['src']             
                
                    event_title.append(title.text)
                    event_location.append(location.text)
                    event_date.append(date.text.strip())
                    event_buy.append(link)
                ...
                try:
                    ...
                    for x in range(0, int(article['results'])):
                        EventResult.objects.create(
                            ...
                            location = article['location'][x],
                            date = article['date'][x],
                            link = article['link'][x],
                            image = article['img'][x]
                        )
                except Exception as e:
                    print('failed at movie_list is none')
                    print(e)
    return event_results

# This is where the magic begins
def FetchMovieResults(Search):
...
#for events in FetchEvents():
#    print("Results: "+ events['results'] + "\n")
#    for x in range(0, int(events['results'])):
#        print(events['title'][x] + " | " + events['date'][x] + " | " + events['location'][x] + " | " + events['link'][x] + "\n")
# =======================================================================================================#